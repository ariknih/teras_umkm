...
export default function HomeExplorer({ products, services }: HomeExplorerProps) {
  ...
  return (
    <section className="w-full max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      {/* ── HEADER SWITCHER BAR ── */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="space-y-1 text-center md:text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200/60 dark:border-emerald-800/40 rounded-full text-emerald-800 dark:text-emerald-300 text-xs font-bold tracking-wide">
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            Temukan aneka produk lokal unggulan atau pesan jasa profesional terpercaya.
          </p>
        </div>

        {/* Segmented Switcher Controls */}
        <div className="bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl flex items-center gap-1.5 shadow-inner shrink-0 w-full sm:w-auto">
          <button
            ...
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2.5 px-6 py-3 rounded-xl font-bold text-xs sm:text-sm transition-all duration-200 cursor-pointer ${
              activeTab === 'MARKETPLACE'
                ? 'bg-white dark:bg-slate-900 text-[#006e24] dark:text-emerald-400 shadow-md ring-1 ring-black/5 font-extrabold'
          ...
          >
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
              activeTab === 'MARKETPLACE' ? 'bg-emerald-100 text-[#006e24]' : 'bg-slate-200 text-slate-600'
            }`}>
          ...
          >
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
              activeTab === 'JASA' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-600'
            }`}>
              {services.length}
            </span>
          </button>
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl pl-11 pr-10 py-3.5 text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-[#006e24] focus:ring-2 focus:ring-[#006e24]/10 shadow-xs"
          />
          {searchQuery && (
            <button
        ...
        <div className="space-y-4 pt-2">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-400">({filteredProducts.length} produk ditemukan)</span>
            </h3>
            <Link
              href="/market"
              className="text-xs font-bold text-[#006e24] hover:text-[#0a3822] hover:underline inline-flex items-center gap-1"
            >
          ...
          {filteredProducts.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 p-8 space-y-3">
              ...
              <h4 className="font-bold text-slate-900 dark:text-white text-sm">Produk tidak ditemukan</h4>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3.5 sm:gap-4">
              {filteredProducts.map((product) => (
                <Link
                  ...
                  className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden shadow-xs hover:shadow-md hover:border-emerald-300 dark:hover:border-emerald-700 transition-all duration-200 flex flex-col justify-between group"
                >
                  <div>
                    {/* Thumbnail Image */}
                    <div className="w-full aspect-square bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                      {product.imageUrl ? (
                        <img
                      ...
                      <span className="absolute top-2 left-2 bg-black/60 backdrop-blur-md text-white text-[9px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider">
                        {product.category}
                      </span>
                    </div>
                    ...
                    <div className="p-3 space-y-1.5">
                      <h4 className="text-xs font-bold text-slate-900 dark:text-white line-clamp-2 leading-snug group-hover:text-[#006e24] transition-colors">
                        {product.title}
                      </h4>
                      <p className="text-sm font-extrabold text-[#006e24] dark:text-emerald-400 font-mono">
                  ...
                  <div className="p-3 pt-0">
                    <span className="block w-full text-center py-2 bg-emerald-50 hover:bg-[#006e24] text-[#006e24] hover:text-white dark:bg-emerald-950/40 dark:text-emerald-300 dark:hover:bg-[#006e24] text-[11px] font-bold rounded-xl transition-colors">
                      Lihat Produk
                    </span>
                  </div>
                </Link>
        ...
        <div className="space-y-4 pt-2">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-400">({filteredServices.length} penyedia aktif)</span>
            </h3>
            <Link
              href="/jasa"
              className="text-xs font-bold text-[#006e24] hover:text-[#0a3822] hover:underline inline-flex items-center gap-1"
          ...
          {filteredServices.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 p-8 space-y-3">
              ...
              <h4 className="font-bold text-slate-900 dark:text-white text-sm">Layanan belum tersedia</h4>
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
              {filteredServices.map((svc) => {
                ...
                return (
                  <div
                    key={svc.id}
                    className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden shadow-xs hover:shadow-lg hover:border-emerald-300 dark:hover:border-emerald-700 transition-all duration-200 flex flex-col justify-between group"
                  >
                    <div>
                      {/* Image Container */}
                      <div className="h-44 bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                        <img
                          ...
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                        />
                        <span className="absolute top-2.5 left-2.5 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                          {svc.category}
                        </span>
                        {svc.location && (
                      ...
                      <div className="p-4 space-y-3">
                        <h4 className="font-bold text-slate-900 dark:text-white text-sm line-clamp-1 group-hover:text-[#006e24] transition-colors">
                          {svc.title}
                        </h4>
                        ...
                        <div className="space-y-1.5 pt-1 border-t border-slate-100 dark:border-slate-800">
                          {svc.pricePerSession ? (
                            <div className="flex items-center justify-between text-xs bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-1 rounded-lg border border-emerald-200/50 dark:border-emerald-800/30">
                              ...
                              <span className="font-mono font-extrabold text-[#006e24] dark:text-emerald-400">
                          ...
                          {svc.pricePerDay ? (
                            <div className="flex items-center justify-between text-xs bg-blue-50 dark:bg-blue-950/40 px-2.5 py-1 rounded-lg border border-blue-200/50 dark:border-blue-800/30">
                    ...
                    <div className="p-4 pt-0">
                      <Link
                        href={`/jasa/${svc.id}`}
                        className="w-full py-2.5 bg-[#006e24] hover:bg-[#0a3822] text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow-xs"
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
