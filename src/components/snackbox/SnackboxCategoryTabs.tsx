          ...
          return (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all border shrink-0 cursor-pointer ${
                isActive
                  ? 'bg-on-primary-container text-white border-on-primary-container shadow-xs'
                  : 'bg-white text-gray-700 border-gray-200 hover:border-on-primary-container/50 hover:text-gray-900'
              }`}
            >
              <span>{cat === 'Semua' ? 'Semua' : formatCategoryName(cat)}</span>
