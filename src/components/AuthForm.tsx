...
export function AuthForm({ defaultTab = "login", onSuccess }: AuthFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (tab === "register") {
      // Registration requires a real email (business identity), unlike login
      // which also accepts a username per the login server action.
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5 mt-1">
        {tab === "register" && (
          <>
            <div className="space-y-1 animate-in fade-in slide-in-from-top-2 duration-300">
              <Label htmlFor="auth-name" className="text-xs font-bold text-slate-700">Nama Lengkap</Label>
              <Input
                id="auth-name"
                type="text"
                required
            ...
            <div className="space-y-1 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="flex justify-between items-center">
                <Label htmlFor="auth-username" className="text-xs font-bold text-slate-700">Username</Label>
                {username && (
        ...
        <div className="space-y-1">
          <Label htmlFor="auth-email" className="text-xs font-bold text-slate-700">
            {tab === "login" ? "Email atau Username" : "Email"}
          </Label>
          <Input
            id="auth-email"
            ...
            placeholder={tab === "login" ? "username_anda atau nama@email.id" : "penya-user.id@gmail.com"}
            className="pl-4 py-2.5 rounded-xl border border-slate-200 focus:border-primary focus:ring-1 focus:ring-primary/20 text-xs"
          />
        </div>
        ...
        {tab === "register" && (
          <>
            <div className="space-y-1 animate-in fade-in slide-in-from-top-2 duration-300">
              <Label htmlFor="auth-role" className="text-xs font-bold text-slate-700">Tipe Akun</Label>
              <select
                id="auth-role"
                value={role}
            ...
            {role === "MERCHANT" && (
              <div className="space-y-1 animate-in fade-in slide-in-from-top-2 duration-300">
                <Label htmlFor="auth-community" className="text-xs font-bold text-slate-700">Komunitas Induk</Label>
            ...
            {/* Referral Code Block */}
            <div className="space-y-1 animate-in fade-in slide-in-from-top-2 duration-300">
              <Label htmlFor="auth-referral" className="text-xs font-bold text-slate-700">Kode Referral (Opsional)</Label>
              <Input
                id="auth-referral"
                type="text"
                value={referralCode}
