...
export default function FloatingChat() {
  ...
  if (!currentUser || currentUser.role === 'CUSTOMER_SERVICE') {
  ...
  return (
    <div className="fixed bottom-24 right-4 md:bottom-6 md:right-6 z-50 font-geist">
      ...
      <AnimatePresence>
        {showMenu && !isOpen && (
          <motion.div
            ...
            className="absolute bottom-16 right-0 w-48 bg-white/95 backdrop-blur-md border border-[#c6a96b]/35 rounded-2xl p-2.5 shadow-2xl flex flex-col gap-1 z-50"
          >
            {/* Livechat Option */}
            <button
              onClick={() => {
              ...
              className="flex items-center gap-2.5 p-2.5 rounded-xl hover:bg-[#c6a96b]/10 text-left transition-colors cursor-pointer w-full border-none bg-transparent group"
            >
              <div className="w-6 h-6 rounded-lg bg-[#c6a96b]/10 text-[#c6a96b] flex items-center justify-center text-xs group-hover:bg-[#c6a96b] group-hover:text-white transition-colors shrink-0">
                💬
              </div>
              <div className="leading-tight">
                <span className="block text-[10px] font-extrabold text-zinc-800">Livechat Teras</span>
                <span className="block text-[7px] text-zinc-400 font-bold tracking-tight uppercase">Platform Support</span>
              </div>
            </button>
            ...
            >
              ...
              <div className="leading-tight">
                <span className="block text-[10px] font-extrabold text-zinc-800">WhatsApp Chat</span>
            ...
            >
              ...
              <div className="leading-tight">
                <span className="block text-[10px] font-extrabold text-zinc-800">Telegram Support</span>
      ...
      <motion.button
        ...
        className="w-14 h-14 bg-gradient-to-tr from-[#c6a96b] to-[#e5c178] border border-[#c6a96b]/35 text-white rounded-full shadow-[0_8px_30px_rgba(198,169,107,0.3)] flex items-center justify-center relative cursor-pointer group"
      >
        {isOpen || showMenu ? (
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-18 right-0 w-[calc(100vw-32px)] sm:w-[420px] max-w-[420px] h-[520px] max-h-[80vh] sm:max-h-none bg-white/95 backdrop-blur-xl border border-[#c6a96b]/30 rounded-2xl shadow-[0_12px_40px_rgba(115,92,0,0.12)] flex overflow-hidden"
          >
            {/* Left sidebar: Conversations list */}
            <div 
              className={`w-full sm:w-1/3 border-r border-[#c6a96b]/20 bg-zinc-50/90 flex flex-col justify-between ${
                activeMobileView === 'list' ? 'flex' : 'hidden sm:flex'
              }`}
            >
              <div className="flex-1 overflow-y-auto">
                <div className="p-3 border-b border-zinc-200 bg-[#c6a96b]/5">
                  <h4 className="text-[10px] font-extrabold text-[#c6a96b] uppercase tracking-widest">Pesan Masuk</h4>
                </div>
                ...
                <button
                  ...
                  className={`w-full p-2.5 flex items-center gap-2 border-b border-zinc-100 text-left transition-colors ${
                    isCsMode ? 'bg-[#c6a96b]/15' : 'hover:bg-zinc-100'
                  }`}
                >
                  <div className="w-7 h-7 rounded-full bg-[#c6a96b]/10 flex items-center justify-center text-[#c6a96b] font-bold text-xs">
                    🎧
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-bold text-zinc-800 truncate">Bantuan CS</p>
                    <p className="text-[8px] text-zinc-500 truncate font-medium">Hubungi Customer Service</p>
                  </div>
                </button>

                {/* Conversation List */}
                {conversations.length === 0 ? (
                  <p className="text-[9px] text-zinc-400 text-center mt-12 p-3 font-medium">Belum ada obrolan.</p>
                ) : (
                  conversations.map((conv) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full p-2.5 flex items-center gap-2 border-b border-zinc-100 text-left transition-colors ${
                          isActive ? 'bg-[#c6a96b]/15' : 'hover:bg-zinc-100'
                        }`}
                      >
                        <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-[#c6a96b] to-[#e5c178] flex items-center justify-center text-white font-bold text-[10px]">
                          {recipient?.name?.charAt(0).toUpperCase() || 'U'}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-center">
                            <p className="text-[10px] font-bold text-zinc-800 truncate">{recipient?.name}</p>
                            {conv.unreadCount > 0 && (
                              <span className="w-3.5 h-3.5 bg-[#c6a96b] text-white font-extrabold text-[8px] rounded-full flex items-center justify-center">
                                {conv.unreadCount}
                              </span>
                            )}
                          </div>
                          <p className="text-[8px] text-zinc-500 truncate font-light mt-0.5">
              ...
              <div className="p-2 border-t border-zinc-200 bg-zinc-100/50">
                <button
                  type="button"
                  onClick={handleOpenCsCreation}
                  className="w-full py-1.5 bg-[#c6a96b]/10 hover:bg-[#c6a96b]/20 border border-[#c6a96b]/30 text-[#c6a96b] font-geist font-bold text-[9px] uppercase tracking-wider rounded transition-colors"
                >
                  🎧 Ajukan Tiket CS
                </button>
              </div>
            ...
            <div 
              className={`w-full sm:w-2/3 flex flex-col justify-between bg-zinc-50/30 ${
                activeMobileView === 'chat' ? 'flex' : 'hidden sm:flex'
              }`}
            >
              {/* Header */}
              <div className="p-3 bg-white border-b border-zinc-200 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="sm:hidden p-1 text-[#c6a96b] hover:text-[#b09259] text-xs font-bold font-geist"
                  >
                    ← Kembali
                  </button>
                  <div>
                    ...
                    <p className="text-[7px] font-mono text-zinc-400 uppercase tracking-widest mt-0.5 font-semibold">
                      {isCsMode 
                ...
                {!isCsMode && activeRoom?.product && (
                  <div className="text-[8px] bg-[#c6a96b]/10 border border-[#c6a96b]/20 px-2 py-0.5 rounded text-[#c6a96b] font-semibold truncate max-w-[100px]">
              ...
              <div ref={scrollRef} className="flex-1 p-3 overflow-y-auto space-y-3 bg-[#FAF8F5]/60">
                {isCsMode && !csTicket ? (
                  <div className="h-full flex flex-col items-center justify-center p-6 text-center">
                    <span className="text-3xl mb-3">🎧</span>
                    <p className="text-xs font-bold text-zinc-800 mb-1">Butuh bantuan operasional?</p>
                    ...
                    <button
                      type="button"
                      onClick={handleOpenCsCreation}
                      className="px-4 py-2 bg-[#c6a96b] hover:bg-[#b09259] text-white font-geist font-bold text-[10px] uppercase tracking-wider rounded transition-all shadow-md shadow-[#c6a96b]/20"
                    >
                      Buka Tiket CS
                    </button>
                  </div>
                  ...
                  messages.map((m) => {
                    ...
                    if (isSystem) {
                      return (
                        <div key={m.id} className="flex justify-center my-2">
                          <span className="bg-[#c6a96b]/10 border border-[#c6a96b]/20 text-[#c6a96b] text-[8px] font-medium px-2 py-1 rounded max-w-[85%] text-center leading-relaxed">
                            📢 {m.content}
                          </span>
                        </div>
                      )
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`max-w-[80%] rounded-xl p-2.5 shadow-sm text-xs leading-relaxed ${
                            isMe
                              ? 'bg-gradient-to-br from-[#c6a96b]/20 to-[#c6a96b]/10 border border-[#c6a96b]/30 text-zinc-900 rounded-tr-none'
                        ...
                        >
                          ...
                          <div className="flex justify-end items-center gap-1 mt-1 text-[7px] text-zinc-400 font-semibold">
                ...
                {isTyping && (
                  <div className="flex justify-start">
                    <div className="bg-white border border-zinc-200 shadow-sm rounded-xl px-3 py-2 text-[10px] text-zinc-400 font-medium">
                      Sedang mengetik...
                    </div>
                  </div>
                )}
              ...
              {(activeRoom || (isCsMode && csTicket)) && (
                <div className="p-3 bg-white border-t border-zinc-200 space-y-2">
                  {uploadImage && (
                    <div className="relative inline-block border border-zinc-200 p-1 rounded bg-zinc-50">
                      <img src={uploadImage} alt="preview" className="h-10 w-10 object-cover rounded" />
                      <button
                        type="button"
                  ...
                  <form onSubmit={handleSendChat} className="flex gap-2">
                    <button
                      ...
                      className="p-2 border border-zinc-200 bg-zinc-50 hover:bg-zinc-100 hover:border-[#c6a96b]/30 rounded hover:text-[#c6a96b] transition-colors text-xs flex items-center justify-center shrink-0 cursor-pointer"
                      title="Upload Image"
                    >
                      📷
                    </button>
                    ...
                    <input
                      ...
                      className="flex-1 px-3 py-1.5 bg-zinc-50 border border-zinc-200 rounded text-xs text-zinc-800 placeholder:text-zinc-400 focus:outline-none focus:border-[#c6a96b]/40 font-medium"
                    />
                    <button
                      type="submit"
                      className="px-4 py-1.5 bg-[#c6a96b] hover:bg-[#b09259] text-white font-geist font-bold text-xs uppercase rounded transition-colors shrink-0 cursor-pointer shadow-md shadow-[#c6a96b]/10"
                    >
                      Kirim
                    </button>
                  </form>
        ...
        {isTicketModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <motion.div
              ...
              className="w-full max-w-md bg-white border border-[#c6a96b]/30 rounded-xl p-6 shadow-[0_20px_50px_rgba(115,92,0,0.15)]"
            >
              <div className="flex items-center gap-3 mb-4 border-b border-zinc-100 pb-3">
                <span className="w-8 h-8 rounded-full bg-[#c6a96b]/10 flex items-center justify-center text-[#c6a96b] text-sm">
                  🎧
                </span>
                <div>
                  <h3 className="font-sora text-sm font-bold text-zinc-800">Buka Tiket Bantuan CS</h3>
                  <p className="text-[8px] text-zinc-400 font-geist uppercase tracking-wider mt-0.5 font-bold">Teras Customer Service</p>
                </div>
              </div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 focus:border-[#c6a96b]/40 rounded-lg text-xs text-zinc-800 placeholder:text-zinc-400 outline-none resize-none mb-4 transition-all font-medium"
              />
              ...
              <div className="flex justify-end gap-2 text-xs font-geist">
                <button
                  ...
                  className="px-4 py-2 border border-zinc-200 hover:bg-zinc-100 text-zinc-500 font-bold rounded-lg transition-colors cursor-pointer"
                >
                  Batal
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-[#c6a96b] hover:bg-[#b09259] text-white font-extrabold rounded-lg transition-colors cursor-pointer shadow-md shadow-[#c6a96b]/15"
                >
                  Ajukan Tiket
                </button>
              </div>
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-24 right-6 z-50 px-4 py-3 bg-[#c6a96b] text-white font-bold text-xs rounded-lg shadow-2xl flex items-center gap-2"
          >
            <span>✓</span>
